...
const { argv } = yargs
  ...
  .version(version)
  .epilogue(
    "For more information, check the wiki at https://github.com/obartra/ssim/wiki/CLI"
  );

function threshold(value, limit, quiet) {
  if (value >= limit) {
    log(
      `SSIM value of ${value.toPrecision(4)} is above the threshold ${
        limit
      }. Success!`,
      quiet
    );
    ...
    log(
      `Error, SSIM value of ${value.toPrecision(4)} not above the threshold ${
        limit
      }.`,
      quiet
    );
