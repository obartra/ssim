...
test('should read image dimensions correctly', t =>
  readpixels('./spec/samples/lena/color.jpg', Promise).then(pixels => {
    t.equals(pixels.width, 512)
    t.equals(pixels.height, 512)
  }))

test('should downsize images when a limit parameter is specified', t =>
  readpixels('./spec/samples/lena/color.jpg', Promise, 100).then(pixels =>
    t.equal(Math.min(pixels.width, pixels.height), 100)
  ))
...
test('should correctly read JPG data', t =>
  loaded.then(pixels => t.deepEqual(pixels.gradientJPG, gradientData)))

test('should correctly read PNG data', t =>
  loaded.then(pixels => t.deepEqual(pixels.gradientPNG, gradientData)))

test('should correctly read GIF data', t =>
  loaded.then(pixels => t.deepEqual(pixels.gradientGIF, gradientData)))

test('should correctly read BMP data', t =>
  loaded.then(pixels => t.deepEqual(pixels.gradientBMP, gradientData)))
...
test('should normalize 1bit data to 8', t =>
  loaded.then(pixels => t.equal(maxPixel(pixels.bit1), 255)))

test('should normalize 2bit data to 8', t =>
  loaded.then(pixels => t.equal(maxPixel(pixels.bit2), 255)))

test('should normalize 4bit data to 8', t =>
  loaded.then(pixels => t.equal(maxPixel(pixels.bit4), 255)))

test('should keep 8bit data at 8 bits', t =>
  loaded.then(pixels => t.equal(maxPixel(pixels.bit8), 255)))

test('should normalize 24bit data to 8', t =>
  loaded.then(pixels => t.equal(maxPixel(pixels.bit24), 255)))
