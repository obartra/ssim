...
test('should produce the right SSIM for bmp images as well (avion)', t =>
  index(samples.avion, samples.avion_j2000_r1).then(({ mssim }) =>
    t.equal(roundTo(mssim, 5), 0.9788)
  ))
...
test('downsizing should produce comparable results between "fast" and "original"', t =>
  Promise.all([
    ...
    t.equal(
      Math.abs(fastMssim - origMssim) < 0.05,
      true,
...
test('ssim should produce the same output than originalSsim', t =>
  index(samples.avion, samples.avion_j2000_r1, {
    ssim: 'fast',
  }).then(({ mssim: fast }) => {
    index(samples.avion, samples.avion_j2000_r1, { ssim: 'original' })
      .then(({ mssim: original }) => {
        t.equal(
          roundTo(fast, 5),
          roundTo(original, 5),
          'SSIM should produce the same results'
        )
      })
      .catch(t.fail)
  }))
...
const lena = getJSONScores(
  lenaScores,
  resolve(__dirname, '../samples/lena'),
  'gif'
)

Object.keys(lena).forEach(key => {
  test(`should get a mssim of ${lena[key].mssim} for ${key}`, t =>
    compare(lena[key], t))
})

const ar = getJSONScores(
  arScores,
  resolve(__dirname, '../samples/aspectratio'),
  'jpg'
)

Object.keys(ar).forEach(key => {
  test(`should get a mssim of ${ar[key].mssim} for ${key}`, t =>
    compare(ar[key], t))
})
...
test('should downsample images and produce somewhat similar results', t => {
  ...
  Promise.all([ssimMap, fullSsimMap])
    .then(([ssimResults, fullSsimResults]) => {
