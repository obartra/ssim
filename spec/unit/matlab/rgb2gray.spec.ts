...
describe('rgb2gray', () => {
  ...
  test('should calculate luminance for RGB (int)', () => {
    const luma = rgb2grayInteger({ data: [100, 200, 50], width: 3, height: 1 })
      .data[0]
  ...
  test('should weight each color based on the ITU spec (int)', () => {
    const rLuma = rgb2grayInteger({ data: [100, 0, 0], width: 3, height: 1 })
      .data[0]
    const gLuma = rgb2grayInteger({ data: [0, 100, 0], width: 3, height: 1 })
      .data[0]
    const bLuma = rgb2grayInteger({ data: [0, 0, 100], width: 3, height: 1 })
      .data[0]
