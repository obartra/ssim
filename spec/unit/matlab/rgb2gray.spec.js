...
test('should calculate luminance for RGB', (t) => {
	const luma = rgb2gray([[[100, 200, 50]]])[0][0];
...
test('should return the same value if all pixels are the same', (t) => {
	const luma = rgb2gray([[[200, 200, 200]]])[0][0];
...
test('should weight each color based on the ITU spec', (t) => {
	const rLuma = rgb2gray([[[100, 0, 0]]])[0][0];
	const gLuma = rgb2gray([[[0, 100, 0]]])[0][0];
	const bLuma = rgb2gray([[[0, 0, 100]]])[0][0];
