...
describe("IVC", () => {
  ...
  it("should match stored mssims (ssim fast) - rgb int", async () => {
    const expected = await getJSONScores(scores, path, "bmp");
    const results = Object.entries(expected)
      .map(([key, { file, reference }]): [string, number] => {
    ...
    expect(scores as MSSIMValues).toMatchSnapshot();
  }, 500000);

  it("should match stored mssims (weber) - rgb int", async () => {
    const expected = await getJSONScores(weberScores, path, "bmp");
    const start = new Date().getTime();
    const results = Object.entries(expected)
      .map(([key, { file, reference }]): [string, number] => {
    ...
    expect(roundTo(newMean, 4)).toMatchInlineSnapshot(`0.0209`);
    expect(roundTo(newVar, 6)).toMatchInlineSnapshot(`0.00021`);
    expect(results).toMatchSnapshot();
  }, 70000);

  it("should match stored mssims (bezkrovny) - rgb int", async () => {
    const expected = await getJSONScores(weberScores, path, "bmp");
    const start = new Date().getTime();
    const results = Object.entries(expected)
      .map(([key, { file, reference }]): [string, number] => {
