...
export function weberSumMatrix(
  pImgData1: any,
  pImgData2: any,
  width: number,
  height: number
...
export function weberSsim(
  ...
  const reciprocalWindowSquared = 1 / windowSquared
  {
    ...
    {
      const meanx =
        sumMatrix[bottomOffset + windowOffset] * reciprocalWindowSquared
      const meany =
        sumMatrix[imageSize + bottomOffset + windowOffset] *
        reciprocalWindowSquared
      const varx =
        sumMatrix[2 * imageSize + bottomOffset + windowOffset] *
          reciprocalWindowSquared -
        meanx * meanx
      const vary =
        sumMatrix[3 * imageSize + bottomOffset + windowOffset] *
          reciprocalWindowSquared -
        meany * meany
      const cov =
        sumMatrix[4 * imageSize + bottomOffset + windowOffset] *
          reciprocalWindowSquared -
        meanx * meany
  ...
  for (let h = 1; h < windowHeight; ++h) {
    ...
    for (let w = 1; w < windowWidth; ++w) {
      ...
      const meanx =
        (sumMatrix[topOffset + leftOffset] -
          sumMatrix[topOffset + rightOffset] -
          sumMatrix[bottomOffset + leftOffset] +
          sumMatrix[bottomOffset + rightOffset]) *
        reciprocalWindowSquared
      const meany =
        (sumMatrix[imageSize + topOffset + leftOffset] -
          sumMatrix[imageSize + topOffset + rightOffset] -
          sumMatrix[imageSize + bottomOffset + leftOffset] +
          sumMatrix[imageSize + bottomOffset + rightOffset]) *
        reciprocalWindowSquared
      const varx =
        (sumMatrix[2 * imageSize + topOffset + leftOffset] -
          sumMatrix[2 * imageSize + topOffset + rightOffset] -
          sumMatrix[2 * imageSize + bottomOffset + leftOffset] +
          sumMatrix[2 * imageSize + bottomOffset + rightOffset]) *
          reciprocalWindowSquared -
        meanx * meanx
      const vary =
        (sumMatrix[3 * imageSize + topOffset + leftOffset] -
          sumMatrix[3 * imageSize + topOffset + rightOffset] -
          sumMatrix[3 * imageSize + bottomOffset + leftOffset] +
          sumMatrix[3 * imageSize + bottomOffset + rightOffset]) *
          reciprocalWindowSquared -
        meany * meany
      const cov =
        (sumMatrix[4 * imageSize + topOffset + leftOffset] -
          sumMatrix[4 * imageSize + topOffset + rightOffset] -
          sumMatrix[4 * imageSize + bottomOffset + leftOffset] +
          sumMatrix[4 * imageSize + bottomOffset + rightOffset]) *
          reciprocalWindowSquared -
        meanx * meany
      const na = 2 * meanx * meany + c1
      const nb = 2 * cov + c2
